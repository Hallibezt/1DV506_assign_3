...
public String getDate(boolean fullYear) {
			...
			if(this.year % 100 == 0) {
				if(this.year % 400 == 0) {
					if(this.day > 29) {//if year  div. by 4,100 and 400
						return date = "Invalid date - leap year, 29 days in February";
						}
				}
				if(this.day > 28) {//if year  div. by 4 and 100 but not 400
					return date = "Invalid date - only 28 days in February";
					}
			}
	...
	if (this.month == 4 || this.month == 6 || this.month == 9 || this.month == 11) {
		if(day > 30) {
		return date = "Invalid date - only 30 days in this month";
		}
	}
